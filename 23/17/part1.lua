...
local function astar(start_node, done, neighbors, heuristic, eq)
...
function Node.new(x, y, dir, steps)
    if steps > 3 or x < 1 or y < 1 or x > width or y > height then
        return nil
    end
...
print(
    astar(
        Node.new(1, 1, 1, 0),
        function(node) return node.x == width and node.y == height end,
        Node.neighbors,
